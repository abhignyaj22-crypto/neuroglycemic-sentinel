            ...
            rows.append(
                {
                    ...
                    "eeg_available_time": anchor if eeg_available else None,
                    "eeg_clock_uncertainty_ms": 0.5,
                    ...
                    "wearable_available_time": anchor,
                    "wearable_clock_uncertainty_ms": 1.0,
...
def test_aligned_loader_patient_split_and_batch_masks(tmp_path: Path) -> None:
    ...
    patient_mass: dict[str, float] = {}
    for batch in batches:
        for patient, weight in zip(
            batch["patient_ids"], batch["sample_weight"].tolist(), strict=True
        ):
            patient_mass[patient] = patient_mass.get(patient, 0.0) + weight
    assert max(patient_mass.values()) == pytest.approx(min(patient_mass.values()))
...
def test_available_time_and_clock_uncertainty_are_mandatory(tmp_path: Path) -> None:
    missing_time = _aligned_fixture().drop(columns=["eeg_available_time"])
    with pytest.raises(ValueError, match="eeg_available_time"):
        load_aligned_window_frame(_write(missing_time, tmp_path / "missing_time.csv"), (30, 60))

    unknown_clock = _aligned_fixture()
    unknown_clock.loc[0, "wearable_clock_uncertainty_ms"] = np.nan
    with pytest.raises(ValueError, match="wearable_clock_uncertainty_ms"):
        load_aligned_window_frame(_write(unknown_clock, tmp_path / "clock.csv"), (30, 60))


@pytest.mark.parametrize(
    "unsafe_name",
    ["eeg_target_glucose_30m_mg_dl", "eeg_future_glucose_mg_dl"],
)
def test_modality_prefixed_label_leakage_is_rejected(
    tmp_path: Path, unsafe_name: str
) -> None:
    frame = _aligned_fixture()
    frame[unsafe_name] = frame["target_glucose_30m_mg_dl"]
    with pytest.raises(ValueError, match="Unsafe model features"):
        load_aligned_window_frame(_write(frame, tmp_path / "unsafe.csv"), (30, 60))
