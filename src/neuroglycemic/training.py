...
CHECKPOINT_SCHEMA = "neural-glucose-checkpoint-v1"
...
def _validate_config(config: NeuralTrainingConfig) -> None:
    ...
    if config.forecast_mode not in {"ambient_no_cgm", "announced_meal_no_cgm"}:
        raise ValueError(
            "forecast_mode must be 'ambient_no_cgm' or 'announced_meal_no_cgm'."
        )
    if config.input_cgm:
        raise ValueError(
            "The primary non-invasive experiment requires input_cgm=false; "
            "use a separately named CGM-augmented experiment otherwise."
        )
    ...
    with context:
        for raw_batch in batches:
            ...
            for name, value in output.components.items():
                ...
                component_sums[name] = component_sums.get(name, 0.0) + component_value * examples

    if total_examples == 0:
        raise ValueError("Training and validation iterables must not be empty.")
    result = {"loss": loss_sum / total_examples}
    result.update(
        {name: value / total_examples for name, value in component_sums.items()}
    )
    if training:
