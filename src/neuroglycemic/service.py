...
def build_neural_checkpoint_metadata(
    ...
    *,
    ...
    model_version: str = "neuroglycemic-neural-v1",
) -> dict[str, Any]:
...
class NeuralGlucoseService:
    ...
    def from_checkpoint(
        ...
        *,
        expected_prediction_target: str = NEURAL_PREDICTION_TARGET,
        ...
        if training_config.get("prediction_target") != expected_prediction_target:
            raise ValueError(
                "Checkpoint prediction target does not match the service target."
            )
        if training_config.get("input_cgm") is not False:
            raise ValueError(
                "This non-invasive service requires an explicit input_cgm=false contract."
            )
        ...
        if not isinstance(configured_model, dict) or any(
            not math.isclose(
                float(configured_model.get(name, float("nan"))), float(value)
            )
        ...
        loaded = load_neural_checkpoint(
            ...
            expected_prediction_target=expected_prediction_target,
        ...
        return cls(
            model,
            prediction_target=expected_prediction_target,
    ...
    def forecast(
        ...
        with torch.inference_mode():
            standardized_outputs = self.model(
                feature_tensors, mask_tensors, availability, quality, staleness
            )
        ...
        weights = outputs["fusion_weights"][0].detach().cpu()
        ...
        if abstained:
            ...
            lower = predicted - 1.96 * standard_deviation
            upper = predicted + 1.96 * standard_deviation
            sqrt_two = math.sqrt(2.0)
            horizon_means = outputs["expert_mean"][0, :, horizon_index]
            horizon_scales = outputs["expert_scale"][0, :, horizon_index]
            horizon_weights = outputs["fusion_weights"][0]
            ...
            warnings = (
                "Research forecast; the interval reflects model uncertainty and is not a clinical alarm.",
            )
